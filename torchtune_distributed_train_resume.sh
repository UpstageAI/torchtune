...
#!/bin/bash
BASE_DIR="/app-home/eric/01_workspace/upstage_ai/docev_dp/torchtune"  # FIXME : torchtune directory
...
# ============ 설정 변수 ============
NNODES=7 # FIXME : 학습에 사용되는 노드 수
MASTER_NODE="upstage-node-001"   # FIXME : 마스터 노드 이름
MASTER_ADDR="192.168.1.2"        # FIXME : 마스터 노드 내부 IP
MASTER_PORT=39550                 # FIXME : 마스터 노드 포트
NPROC_PER_NODE=8                  # FIXME : 노드당 프로세스 수
NODES=("upstage-node-001" "upstage-node-002" "upstage-node-003" "upstage-node-005" "upstage-node-006" "upstage-node-007" "upstage-node-008")  # FIXME : 학습에 사용되는 GPU 노드 목록

# NCCL 네트워크 인터페이스 설정
NCCL_IFACE="bond-srv.1512"

# 경로 및 환경 설정
CONFIG_PATH="recipes/configs/docev/docev_preview_dp_resume.yaml"  # FIXME : 학습 설정 파일 경로
SCRIPT_PATH="recipes/dev/full_finetune_distributed_ufx_dataset.py"  # FIXME : 학습 스크립트 경로

# 로그 디렉토리 설정
LOG_DIR="${BASE_DIR}/logs"
RUN_NAME="torchtune_distributed_train_docev_dp" # FIXME : 학습 실행 이름
